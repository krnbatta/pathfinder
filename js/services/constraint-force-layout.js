...
export default {
  process(){
    ...
    root = d3.tree()
             .nodeSize([config.nodeSize/20, config.nodeSize/20])
             .size([config.nodeSize * height/8, config.nodeSize * height/8])
             (root);
    root.descendants().forEach((node) => {
      node.data._ids.forEach((_id) => {
        Store.data.Node[_id].variables = {};
        Store.data.Node[_id].variables.x = node.x;
        Store.data.Node[_id].variables.y = node.y;
      });
    });
  },
