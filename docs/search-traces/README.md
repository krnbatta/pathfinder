...
This is a compulsory input to run the app. Download sample [here](http://pf-algo-viz.org/algorithms/astar.json).
...
1. `eventList`: The `eventList` is an array of objects each of which describes the state of a node at a particular stage in a search. Each object in this array is an `event object`.
    ...
    - The information about visual representation of the node is provided using `variables` object. The information provided in this object is used to instantiate different drawing primitives. Here is an example of few cases:
      - For a node which is a point/square, we need their `x` and `y` values as coordinates for drawing a square or a cell. Example file: [here](http://pf-algo-viz.org/algorithms/astar.json)
      ...
      - We can also draw a complex node which is combination of all these nodes. Example of this can be Any angle path finding algorithms. Example file: [Anya](http://pf-algo-viz.org/algorithms/anya.json) and [Polyanya](http://pf-algo-viz.org/algorithms/polyanya.json)
